...
struct PointToShapeDistanceVisitor {
    ...
    double operator()(const Circle &circle) const {
        ...
        return std::max(0.0, std::fabs(center_distance - circle.radius));
    }
...
struct ShapeToShapeDistanceVisitor {
    ...
    std::optional<double> operator()(const Line &l1, const Line &l2) const {
        std::vector<double> distances = {PointToShapeDistanceVisitor{l1.start}(l2), PointToShapeDistanceVisitor{l1.end}(l2),
                                         PointToShapeDistanceVisitor{l2.start}(l1), PointToShapeDistanceVisitor{l2.end}(l1)};
